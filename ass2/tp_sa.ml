...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec names_values = function
    ...
    | _ -> raise (X_syntax "Invalid let ribs structure");;
  ...
  let rec tag_parse sexpr =
    ...
    | ScmPair (ScmSymbol "letrec", ScmPair (ribs, exprs)) ->
       ...
       let dummy_ribs = scheme_sexpr_list_of_sexpr_list (  List.map ( fun name -> ScmPair( name , ScmPair
                                           ( ScmPair (ScmSymbol "quote" , ScmPair(ScmSymbol "whatever" ,ScmNil)),ScmNil))) names )in
       let set_bangs =
             (List.map2
              (fun name value ->
               ScmPair
                 (ScmSymbol "set!" , ScmPair (name , ScmPair(value,ScmNil ))))
              names values) in
