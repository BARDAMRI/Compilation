...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_qq = function
   ...
   let rec macro_expand_cond_ribs ribs =
    ...
    | ScmPair (ScmPair (ScmSymbol "else", exprs), ribs) ->
       (ScmPair (ScmSymbol "begin" , macro_expand_cond_ribs sxprs))
  ...
  let compare_lis args =
  match args with
  | [] -> true
  | car::[] -> true
  | car::cdr -> if List.mem car cdr then false
    else check_diff cdr ;;

  let expend__args args = 
    let args =
      match args with
      | ScmNil -> ScmNil
      | ScmPair(ScmPair(var,exp),cdr) -> ScmPair(var, expend__args cdr)
      | e -> raise X_proper_list_error in
    args ;;

  let rec list_length list = function
    |ScmNil -> 0
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_tail_calls = 
    let rec run in_tail = function
      ...
      | ScmOr' [] -> []
      | ScmOr' (expr :: exprs) -> ScmOr'(runl in_tail expr) ScmOr'(runl in_tail exprs)
      | ScmVarSet' (var', expr') -> ScmSet'(var',runl exprs false)
      | ScmVarDef' (var', expr') -> ScmVarDef'(var',runl exprs false)
      | (ScmBox' _) as expr' -> (ScmBox' _ false) (* idan *) 
      | (ScmBoxGet' _) as expr' -> (ScmBoxGet' _ false) (* idan *)
      | ScmBoxSet' (var', expr') -> ScmBoxSet' (var', run expr' true) (* idan *)
      | ScmLambda' (params, Simple, expr) ->  ScmLambda'(params, Simple , (run expr true))
      | ScmLambda' (params, Opt opt, expr) -> ScmLambda' (pars,par, Opt ,(run body true))
    ...
    and runl in_tail expr = function
      | [] -> [run in_tail expr]
      | expr' :: exprs -> (run false expr) :: (runl in_tail expr' exprs) in
  ...
  let rec auto_box expr =
    ...
    | ScmVarSet' (v, expr) -> ScmVarSet' (v, auto_box expr)  (* idan *)
    | ScmVarDef' (v, expr) -> ScmVarDef (v, auto_box expr)  (* idan *)
    | ScmOr' exprs -> ScmOr' (List.map auto_box exprs) (* idan *)
    | ScmLambda' (params, Simple, expr') ->
       let box_these =
         List.filter
    ...
    | ScmLambda' (params, Opt opt, expr') -> (* idan *)
